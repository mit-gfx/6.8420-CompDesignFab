...
int main(int argc, char* argv[]) {
    // Usage:
    // Slow version:
    // ./assignment1/run1 bunny.stl 2.0
    // ./assignment1/run1 fandisk.stl 0.05
    // ./assignment1/run1 spot.stl 0.125
    // ./assignment1/run1 dragon.stl 0.05
    // Fast version:
    // ./assignment1/run1 bunny.stl 2.0 fast
    // ./assignment1/run1 fandisk.stl 0.05 fast
    // ./assignment1/run1 spot.stl 0.125 fast
    // ./assignment1/run1 dragon.stl 0.05 fast
    // Approximation:
    // ./assignment1/run1 bunny_with_hole.stl 2.0 approx
    // ./assignment1/run1 spot_with_whole.stl 0.125 approx
    // Marching cube version:
    // ./assignment1/run1 bunny_voxel_info.txt
    // ./assignment1/run1 dragon_voxel_info.txt
    // ./assignment1/run1 fandisk_voxel_info.txt
    // ./assignment1/run1 spot_voxel_info.txt
	std::cerr << "Welcome to Computational Design and Fabrication Class" << std::endl;
    ...
    if (argc == 2) {
        ...
        mesh::MarchingCube<double> mc(std::string(PROJECT_SOURCE_DIR) + "/data/assignment1/" + info_file);
        mc.BuildMesh();
        const std::string name = info_file.substr(0, info_file.size() - std::string("_voxel_info.txt").size());
        mc.ExportMeshToFile(std::string(PROJECT_SOURCE_DIR) + "/data/assignment1/results/" + name + "_mc.stl");
        return 0;
    }
    ...
    mesh::Voxelizer<double> voxelizer(std::string(PROJECT_SOURCE_DIR) + "/data/assignment1/" + stl_name, dx);
    int t1 = std::clock();
    ...
    std::string result_dir = std::string(PROJECT_SOURCE_DIR) + "/data/assignment1/results/"; 
...
int run_all() {
    ...
    // Assignment 1.3: voxelization for non-watertight meshes.
